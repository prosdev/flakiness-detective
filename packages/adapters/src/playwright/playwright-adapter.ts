...
import { FileSystemAdapter, FileSystemAdapterConfig } from '../file-system/file-system-adapter';
...
export class PlaywrightAdapter extends FileSystemAdapter implements DataAdapter {
   ...
   */
  async processPlaywrightResults(results: PlaywrightTestResult[]): Promise<TestFailure[]> {
    ...
    const processedFailures = failedTests.map(test => this.parsePlaywrightFailure(test));
   ...
   */
  private extractPlaywrightMetadata(test: PlaywrightTestResult): PlaywrightFailureMetadata {
    const error = test.error || {};
    ...
    if (error.stack) {
      ...
      if (!metadata.filePath || !metadata.lineNumber) {
        const fileLineMatch = error.stack.match(/at\s+([^\s:()]+\.(js|ts|jsx|tsx|vue)):(\d+)/i);
        if (fileLineMatch) {
   ...
   */
  private enhanceMetadataFromErrorMessage(message: string, metadata: PlaywrightFailureMetadata): void {
    // Extract timeout from message if not already set
    if (!metadata.timeoutMs) {
      const timeoutMatch = message.match(/timeout (\d+)ms/i) || message.match(/Timed out (\d+)ms/i);
      if (timeoutMatch) {
    ...
    if (!metadata.selector) {
      const selectorMatch = message.match(/selector ['"]([^'"]+)['"]/i) ||
    ...
    if (!metadata.matcher) {
      ...
      else if (message.includes('to be hidden')) metadata.matcher = 'toBeHidden';
      else if (message.includes('to contain text')) metadata.matcher = 'toContainText';
      else if (message.includes('to have text')) metadata.matcher = 'toHaveText';
      else if (message.includes('to have value')) metadata.matcher = 'toHaveValue';
      else if (message.includes('to be checked')) metadata.matcher = 'toBeChecked';
      else if (message.includes('to be enabled')) metadata.matcher = 'toBeEnabled';
      else if (message.includes('to be disabled')) metadata.matcher = 'toBeDisabled';
    }
    ...
    if (!metadata.filePath || !metadata.lineNumber) {
      ...
      for (const line of lines) {
        ...
        const fileLineMatch = line.match(/at.+\((.+\.(js|ts|jsx|tsx|vue)):(\d+)(:\d+)?\)/i);
        if (fileLineMatch) {
          metadata.filePath = metadata.filePath || fileLineMatch[1];
          metadata.lineNumber = metadata.lineNumber || fileLineMatch[3];
          break;
  ...
  async fetchClusters(limit?: number): Promise<FailureCluster[]> {
    ...
    return clusters.map(cluster => {
      ...
      if (!enhancedCluster.assertionPattern && 
 ...
 */
export function createPlaywrightAdapter(config: PlaywrightAdapterConfig): PlaywrightAdapter {
